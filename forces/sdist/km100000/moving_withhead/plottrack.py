...
for i,(De,Time,Ds,tau) in enumerate(DeTimeDs):
    ...
    if tau in Stau:
        ax1.plot(coarseave(Time,10),coarseave(Ds,10),'.',color = colors[cn],label=r'$De=%.1f$'%(De,))
        ax1.plot(Time,tanfun(Time,popt[0],popt[1],popt[2],popt[3]),'-',color=colors[cn])
        cn+=1
    T0.append(popt[1])
    DE.append(De)
ax2.plot(DE,T0,'b.-')
ax2.set_xlabel('$\mathrm{De}$')
ax2.set_ylabel(r'$\tau_s$')
ax1.set_xlabel('$t$')
