...
zlo = -Lz/2
zhi = Lz/2
Ls = 15.0 #length of the sperm
k = 3.0*pi/Ls
Dxs = 2*np.pi/k/16.0*8
head_pos1 = -Lx/4-Dxs/2 #head position of the flagellum 1
head_pos2 = -Lx/4+Dxs/2 #head position of the flagellum 2
...
ch1 = head_pos1 - 1.2*Rh #center of the head
ch2 = head_pos2 - 1.2*Rh #center of the head

sperm_atoms1 = [[] for i in range(n_layer)]
sperm_atoms2 = [[] for i in range(n_layer)]
xcoords1 = [i*dx+head_pos1 for i in range(0,Ns)]
xcoords2 = [i*dx+head_pos2 for i in range(0,Ns)]
for x in xcoords1:
...
head_region1 = and_region(sphere(make_point(ch1,sdist/2,0),Rh*1.05),box(make_point(-Lx/2,-Ly/2,-Lz/2),make_point(Lx/2,Ly/2,Lz/2)))
head_atoms1 = fill_region(head_region1,make_bound(ch1-ae,ch1+ae,sdist/2-ae,sdist/2+ae,0,0.05),make_part(dx*sqrt(2),dx*sqrt(2),dx*sqrt(2)))

head_region2 = and_region(sphere(make_point(ch2,-sdist/2,0),Rh*1.05),box(make_point(-Lx/2,-Ly/2,-Lz/2),make_point(Lx/2,Ly/2,Lz/2)))
head_atoms2 = fill_region(head_region2,make_bound(ch2-ae,ch2+ae,-sdist/2-ae,-sdist/2+ae,0,0.05),make_part(dx*sqrt(2),dx*sqrt(2),dx*sqrt(2)))


sperm_atoms = [item for sublist in sperm_atoms1 + sperm_atoms2 for item in sublist]+head_atoms1+head_atoms2
tail_region = or_region(box(make_point(head_pos1,sdist/2.0-n_layer/2.0*dx,zlo),make_point(head_pos1+Ls,sdist/2.0+n_layer/2.0*dx,zhi)),box(make_point(head_pos2,-sdist/2.0-n_layer/2.0*dx,zlo),make_point(head_pos2+Ls,-sdist/2.0+n_layer/2.0*dx,zhi)))
...
dr_ml = []
q_m = make_atom(ch1+Rh,sdist/2,0)
for atom,id in zH:
...
dr_ml = []
q_m = make_atom(ch2+Rh,-sdist/2,0)
for atom,id in zH:
    dr_m = atom_dist(atom,q_m)
    dr_ml.append(dr_m)
dr_ml = sorted(enumerate(dr_ml),key=lambda tup: tup[1])
...
Thx1 = [(get_atom_x(atom)-head_pos1)*k for atom in sperm_atoms1[mil]][1:-1]
angle_ntyp += 1

Angles2 = zip(shift(sperm_atoms_index2[mil],-1),shift(sperm_atoms_index2[mil],0),shift(sperm_atoms_index2[mil],1))[1:-1]
Thx2 = [(get_atom_x(atom)-head_pos2)*k+init_phase_diff for atom in sperm_atoms2[mil]][1:-1]
angle_ntyp += 1
