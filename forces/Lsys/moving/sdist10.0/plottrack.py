...
Stau = [0.1,1,2.1,3,7,15,25]
colors = ['b','r','g','y','c','m','k','C0','C1','C2','C3','C4']
linestyles = ['-','--','-.']
pointstyles = ['.','x','+','v','<','s','*']
Time_list = []
Ds_list = []
...
DeTimeDs = sorted(zip(De_list,Time_list,Ds_list))

for cn,(De,Time,Ds) in enumerate(DeTimeDs):
    ax1.plot(omega*np.array(Time),ndm.ndm_length(np.array(Ds)),'-',color=colors[cn],label=r'$\mathrm{De}=%s$'%(myround(De),),linewidth=1)

ax1.legend(loc='best',ncol=3)
ax1.set_xlabel(r'$\omega t$')
ax1.set_ylabel(r'$\Delta D/r_c$')
ax1.set_ylim((ndm.ndm_length(4),ndm.ndm_length(15)))
plt.show()
