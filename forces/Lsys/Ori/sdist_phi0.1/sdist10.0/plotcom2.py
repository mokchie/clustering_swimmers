...
for cn,stau in enumerate(Stau):
    ...
    for infile in infiles:
        ...
        for n in Tau:
            if n<1:
                files1.append(direct+'/data/swimmer1-pos-%.1f.data'%n)
                files2.append(direct+'/data/swimmer2-pos-%.1f.data'%n)
            else:
        ...
        for i,(filename1,filename2) in enumerate(zip(files1,files2)):
            if Tau[i]!=stau:
                continue
            print( 'processing %s & %s... '%(filename1,filename2) )
            XC = []
            YC = []
            for j,filename in enumerate((filename1,filename2)):
                ...
                Xc -= Xc[0]
                Yc -= Yc[0]
                st = len(Time)-int(Tp/dstep)*3
                XC.append(Xc)
                YC.append(Yc)
                Dc = np.sqrt(Xc**2+Yc**2)
                ax1.plot(Xc[st:],Yc[st:],linestyles[j],color=colors[cn],label=r'$\tau=%.1f$'%Tau[i])
