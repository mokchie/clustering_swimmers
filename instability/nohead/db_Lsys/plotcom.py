...
infiles = ['in-%d.run'%i for i in range(1,4)] #in-4.run must be excluded, because some of the simulation didn't finish
IND = [0.0,0.5,0.75,1.0,1.25,1.5,1.55,1.6,1.65,1.7,1.75,2.0,2.25,2.5,3.0,4.0]
...
for cn,dir in enumerate(Dirs):
    for infile in infiles:
        ...
        for fni,tau in zip(fn,Tau):
            Db.append((b2-b1))
            TAU.append(tau)
            if tau<2.0 and np.abs(tau-1)>1e-6:
