...
directs = ['omega_b'+str(i) for i in (6.0,8.0,10.0,12.0,14.0,16.0)]
for cn,direct in enumerate(directs):
    ...
    ax1.plot(ndm.ndm_omega(np.array(Omega)),ndm.ndm_v(np.array(Vx)),'.-',color=colors[cn],label=r'$\theta_b/\pi=%.2f$'%ndm.ndm_angle(b),markersize=8,linewidth=1)

ax1.legend(loc='best',ncol=2)
ax1.set_xlabel(r'$\omega/\omega_\mathrm{ref}$')
ax1.set_ylabel(r'$V/v_\mathrm{ref}$')
ax1.set_ylim((ndm.ndm_v(-0.03),ndm.ndm_v(0.15)))
plt.show()
