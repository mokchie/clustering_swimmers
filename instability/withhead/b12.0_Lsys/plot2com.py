...
for cn,direct in enumerate(Dirs):
    ...
    if cn in [2,3,8,9]:
        continue
    ax1.plot(De,ndm.ndm_v(Vx)*ndm.ndm_k(k)/ndm.ndm_omega(np.abs(omega)),linestyles[cn%2],color=colors[int(cn/2)],label=r'$\omega/\omega_\mathrm{ref}=%s$'%(myround(-omega),),markersize=8,linewidth=1)
...
for i,ns in enumerate(Ns):
    ax2.plot(ndm.ndm_omega(np.array(Om)),ndm.ndm_v(np.array(Vs[:,ns])),'.-',color=colors2[i],label=r'single $\ta/t_\mathrm{ref}u=%.1f$'%myround(ndm.ndm_time(Tau[ns])),markersize=8,linewidth=1)
    ax2.plot(ndm.ndm_omega(np.array(Om)),ndm.ndm_v(np.array(Vp[:,ns])),'*-',color=colors2[i],label=r'pair $\tau/t_\mathrm{ref}=%.1f$'%myround(ndm.ndm_time(Tau[ns])),markersize=8,linewidth=1)
...
lg2 = ax1.legend(handles=legend_elements2,loc='lower right',ncol=2)
