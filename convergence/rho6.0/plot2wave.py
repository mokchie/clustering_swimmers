...
for j,filename in enumerate(files):
    ...
    ax3.plot(Time/(Tp),Hpx1/(Tp/2/np.pi),'b-')
    ax3.plot(Time/(Tp),Hpx2/(Tp/2/np.pi),'r-')
    ...
    FdT = np.array(FdT)
    Fddy = np.array(Fddy)
    ax2.plot(FdT/Tp,Fddy/(Tp/2/np.pi),'k--')
    f = interpolate.interp1d(Time/Tp,Dpx/(Tp/2/np.pi))
    ax2.plot(FdT/Tp,f(FdT/(Tp)),'b-')
    print(trapezoid(np.abs(Fddy/(Tp/2/np.pi)-f(FdT/Tp)),FdT/Tp))
    plt.show()
